# This ReadMe.txt file was generated on 2021/##/## by Michael Cox and Adam Creuziger.

## Abstract

## Readme developed from a NIST Readme Template

# Sections
- GENERAL INFORMATION
- SHARING/ACCESS INFORMATION
- DATA & FILE OVERVIEW
- METHODOLOGICAL INFORMATION
- BIBLIOGRAPHY

# GENERAL INFORMATION
## Title of Dataset: 

## Version History
- 1.0.0 Initial Release

## Author Information

Name: Michael Cox
Institution: Colorado School of Mines 
Address: 1500 Illinois Street, Golden, CO 80401 
Email: mrcox@mymail.mines.edu

Name: Adam Creuziger 
Institution: National Institute of Standards and Technology 
Address: 100 Bureau Drive, Gaithersburg, MD 20899 
Email: adam.creuziger@nist.gov

Other Authors: 
Kip Findley, Colorado School of Mines

## Date of data collection: ##

## Geographic location of data collection: 


## Geographic location of data processing: 
ASPPRC?, Colorado School of Mines, Metallurgical and Materials Engineering Department, Golden, CO 80401

## Funding sources that supported the collection and analysis of the data: 
These datasets were collected in work supported by 
???
  Support for Adam Creuziger was provided by the National Institute of Standards and Technology (NIST), an agency of the Federal Government.

# SHARING/ACCESS INFORMATION

## Licence

## Disclaimers

### National Institute of Standards and Technology (NIST)
The data/work is provided by NIST as a public service and is expressly provided “AS IS.” NIST MAKES NO WARRANTY OF ANY KIND, EXPRESS, IMPLIED OR STATUTORY, INCLUDING, WITHOUT LIMITATION, THE IMPLIED WARRANTY OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, NON-INFRINGEMENT AND DATA ACCURACY. NIST does not warrant or make any representations regarding the use of the data or the results thereof, including but not limited to the correctness, accuracy, reliability or usefulness of the data. NIST SHALL NOT BE LIABLE AND YOU HEREBY RELEASE NIST FROM LIABILITY FOR ANY INDIRECT, CONSEQUENTIAL, SPECIAL, OR INCIDENTAL DAMAGES (INCLUDING DAMAGES FOR LOSS OF BUSINESS PROFITS, BUSINESS INTERRUPTION, LOSS OF BUSINESS INFORMATION, AND THE LIKE), WHETHER ARISING IN TORT, CONTRACT, OR OTHERWISE, ARISING FROM OR RELATING TO THE DATA (OR THE USE OF OR INABILITY TO USE THIS DATA), EVEN IF NIST HAS BEEN ADVISED OF THE POSSIBILITY OF SUCH DAMAGES.

## Are there any restrictions or guidelines on how to use the data? 


## Is there a documentary standard that applies to this dataset? 
No documentary standard has been developed for this dataset.

## Citation Guidance

# DATA & FILE OVERVIEW
This data set has 3 different file types:
- .gpx for diffraction analysis
- .xrdml files for diffraction data


## File List 
1. Filename: XXXXX.gpx
- Description: 
- Notes: 

## Identifiers
BatchIDs

SampleIDs

CollationIDs


## Was data derived from another source? 

# METHODOLOGICAL INFORMATION

## Description of methods used for collection/generation of data:

## Instrument- or software-specific information needed to interpret the data:

## Methods for processing the data: (describe how the submitted data were generated from the raw or collected data) 

## Standards and calibration information, if appropriate:  

## Environmental/experimental conditions:

## Describe any quality-assurance procedures performed on the data: 

## People involved with sample collection, processing, analysis and/or submission:

## DATA-SPECIFIC INFORMATION FOR .gda FILES:

# BIBLIOGRAPHY

